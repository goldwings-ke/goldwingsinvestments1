  ...
  initialize2(){
   ...
   var mBizInfo = this.state.bizinfo.slice();
    for(let x of mBizInfo){
      ...
      const itemsRef = firebase.database().ref('class_group/'+
       uid+'/'+businessKeyId)
       .orderByChild('name');
   ...
   var mBizInfo = this.state.bizinfo.slice();
    ...
    var itemsRef = null;
       if(this.state.isExists){
        itemsRef = firebase.database().ref('class_group/'+uid+'/'+businessKeyId+'/'+this.state.id);
        saved = "Updated!";
        const item = {
          businessKeyId: businessKeyId,
          log: n,
        ...
        itemsRef = firebase.database().ref('class_group/'+uid+'/'+businessKeyId);
          const item = {
            businessKeyId: businessKeyId,
            log: n,
            name: this.state.name,
  ...
  removeItem(itemId) {
   ...
   var mBizInfo = this.state.bizinfo.slice();
    for(let x of mBizInfo){
      ...
      .ref(`/class_group/${uid}/${businessKeyId}/${itemId}`);
      itemRef.remove();
